...
SMB1_PRG_BANKS = 2
SMB1_CHR_BANKS = 1
SMB1_PRG_SIZE = SMB1_PRG_BANKS * 16384
SMB1_CHR_SIZE = SMB1_CHR_BANKS * 8192
SMB1_BODY_SIZE = SMB1_PRG_SIZE + SMB1_CHR_SIZE
SMB1_SIZES = (SMB1_BODY_SIZE, SMB1_BODY_SIZE + INES_HEADER_SIZE)

TEXT_OFFSETS_PRG: dict[str, tuple[int, int]] = {
    "MARIO": (0x0765, 5),
    "LUIGI": (0x07FD, 5),
    "WORLD": (0x076D, 5),
    "TIME": (0x0774, 4),
...
def validate_and_load(path: str) -> tuple[bytearray, bool]:
    ...
    if has_ines:
        ...
        if prg_banks != SMB1_PRG_BANKS or chr_banks != SMB1_CHR_BANKS:
            raise ValueError(
                f"Expected SMB1 mapper ROM (PRG={SMB1_PRG_BANKS}, CHR={SMB1_CHR_BANKS}), "
                f"got PRG={prg_banks}, CHR={chr_banks}"
            )
        expected = INES_HEADER_SIZE + prg_banks * 16384 + chr_banks * 8192
        if len(data) != expected:
            raise ValueError(f"ROM size {len(data)} != expected {expected} for iNES header")
    elif len(data) != SMB1_BODY_SIZE:
        raise ValueError(
            f"Raw ROM must be {SMB1_BODY_SIZE} bytes (or {SMB1_SIZES[1]} with iNES header)"
        )
...
class SMB1Rom:
    ...
    def dump_level_data(self) -> str:
        ...
        for i in range(0, len(headers), 4):
            chunk = headers[i : i + 4]
            lines.append(f"  {i // 4:02d}: {chunk.hex(' ')}")
    ...
    def dump_text(self) -> str:
        lines = ["HUD strings (decode):"]
        for name, (addr, length) in TEXT_OFFSETS_PRG.items():
            raw = self.read_prg(addr, length)
            lines.append(f"  {name}: '{smb_decode(raw)}'  [{raw.hex(' ')}]")
        lines.append("\nEdit lines as NAME=TEXT (5 chars max) then WRITE TO ROM.")
        return "\n".join(lines)
...
class SMB1EditorGUI:
    def __init__(self, root: tk.Tk) -> None:
        self.root = root
        self.root.title("SMB1 UTILITY CLONE // BLUE HUE")
    ...
    def _build_ui(self) -> None:
        ...
        for tab_name, writable in (
            ("Level Data", False),
            ("Enemy Placements", False),
            ("Palette Editor", False),
            ("Text/HUD", True),
            ("Checksum", False),
        ):
    ...
    def load_rom(self) -> None:
        ...
        try:
            ...
            self.status_var.set(
                f"LOADED: {self.rom_path.name} // {len(rom)} bytes // {hdr}"
            )
    ...
    def write_tab(self, name: str) -> None:
        ...
        try:
            if name == "Text/HUD":
                ...
                if n == 0:
                    messagebox.showwarning(
                        "WRITE",
                        "No lines written. Use MARIO=HELLO style (see READ output).",
                    )
                    return
            else:
                return
            ...
            return
        self.modified = True
        self.status_var.set(f"WRITE OK // {name.upper()} ({n} string(s))")
        self.read_tab(name)
